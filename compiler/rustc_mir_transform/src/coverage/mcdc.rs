//! MC/DC instrumentaion enables MC/DC coverage to be performed on the
...
//!     them ([`rustc_middle::mir::coverage::MCDCDecisionSpan`] and
//!     [`MCDCConditionSpan`]) along with the the corresponding spans in the
//!     source code, and stores it in
...
//!     [`rustc_middle::mir::coverage::MCDCDecisionSpan`]s and
//!     [`MCDCConditionSpan`]s to create "meta-mappings", which are essentially
//!     the structures we will give to LLVM but with some extra data we cannot
//!     get rid of just yet. [`extract_mcdc_mappings`] is also responsible for
//!     computing the index increment values of each condition outcome via
...
fn calc_test_vectors_index(conditions: &mut Vec<MCDCMetaConditionMapping>) -> usize {
    ...
    while let Some(branch) = queue.pop_front() {
        let ConditionInfo { condition_id, true_next_id, false_next_id }: ConditionInfo = branch.condition_info;
