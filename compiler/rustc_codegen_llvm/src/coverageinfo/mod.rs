...
impl<'tcx> CoverageInfoBuilderMethods<'tcx> for Builder<'_, '_, 'tcx> {
    ...
    fn init_coverage(&mut self, instance: Instance<'tcx>) {
        let Some(mir_info) = self.tcx.instance_mir(instance.def).coverage_mir_info.as_deref()
        else {
            // Early return if there is no coverage info.
            return;
        };

        let Some(mcdc_info) = mir_info.mcdc_info.as_ref() else {
            return;
        };

        let fn_name = self.ensure_pgo_func_name_var(instance);
        let hash = self.const_u64(mir_info.function_source_hash);
